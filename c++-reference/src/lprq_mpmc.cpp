...
int main(int argc, char *argv[]) {
  ...
  app.add_option("evenCores", evenCores,
                 "If true, use only even numbered cores");
  float congestion_factor = 0.0;
  app.add_option("congestion_factor", congestion_factor,
                 "Congestion factor, 0.0-1.0, 1.0 meaning full congestion");
  ...
  for (int i = 0; i < numProducers; i++) {
    producer_handles[i] =
        std::thread([&tops, &nops, i, &queue, core, &congestion_factor]() {
          // Thread rng
          ...
          CPU_ZERO(&cpuset);
          CPU_SET(core, &cpuset);
          int result = pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t),
                                              &cpuset);
  ...
  for (int i = 0; i < numConsumers; i++) {
    consumer_handles[i] =
        std::thread([i, &queue, core, &done, &congestion_factor]() {
          // Thread rng
          auto engine = std::mt19937(std::random_device{}());
          auto distribution = std::uniform_real_distribution<float>(0.0, 1.0);
          int backoff = 0;
